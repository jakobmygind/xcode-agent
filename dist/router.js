 ...
 */
app.post("/webhook/github", createAuthRateLimitMiddleware(), asyncHandler(async (req, res) => {
    ...
    if (event === "issues" && payload.action === "labeled") {
        ...
        if (label === "agent:opus" || label === "agent:sonnet") {
            ...
            startAgent(repository.owner.login, repository.name, issue.number).catch(error => {
 ...
 */
app.post("/trigger", requireAuth, createAuthRateLimitMiddleware(), asyncHandler(async (req, res) => {
    ...
    // Start agent asynchronously
    startAgent(owner, repo, parseInt(issue, 10), agentType)
        .catch(error => {
        logger.error("Failed to start agent", error, {
            requestId: req.requestId,
        });
 ...
 */
async function startAgent(owner, repo, issueNumber, agentType) {
